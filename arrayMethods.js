...
// CODE HERE
const postTaxPrices = prices.map(price => (price = price *= 1.07));
...
// CODE HERE
const totalPopulation = populations.reduce((total, cur) => total + cur)
...
// CODE HERE
const totalsSum = orders.map(order => order.price + (order.price * order.tax))
...
// CODE HERE
const sumBobsTotal = purchases.filter(owner => purchases.owner === 'Bob').reduce((a, c) => a + c.price, 0)